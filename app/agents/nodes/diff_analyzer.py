...
async def diff_analyzer(state: ReviewState) -> ReviewState:
    ...
    if not title or not author or description is None:
        logger.info(f"[Node: Diff Analyzer] Fetching missing metadata for PR {pr_number}")
    ...
    if not changed_files:
        logger.info(f"[Node: Diff Analyzer] Fetching missing changed files for PR {pr_number}")
    ...
    if not raw_diff:
        logger.info(f"[Node: Diff Analyzer] Fetching missing raw diff for PR {pr_number}")
    ...
    logger.info(f"[Node: Diff Analyzer] Stats: +{stats['total_additions']} -{stats['total_deletions']} in {stats['total_files']} files")
    logger.info(f"[Node: Diff Analyzer] Ignored {stats['ignored_count']} files, Reviewable {stats['reviewable_count']} files")
    ...
    for f_data in reviewable:
        ...
        if parsed_ast:
            ast_summaries[file_path] = parsed_ast.model_dump()
