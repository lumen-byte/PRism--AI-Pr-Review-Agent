...
from app.services.review.review_publisher import review_publisher
...
async def github_publisher(state: ReviewState) -> ReviewState:
    ...
    if not success:
        logger.error(
            f"[Node: GitHub Publisher] Failed to publish review for PR {state.get('pr_number')}"
        )
        state.setdefault("errors", []).append("Failed to publish to GitHub")
    else:
        logger.info("[Node: GitHub Publisher] Successfully published review to GitHub.")
