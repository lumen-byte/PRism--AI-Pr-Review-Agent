...
from app.db.models import Repository, PullRequest, PRStatus, Review, ReviewComment, CommentType, Severity as DBSeverity
...
async def orchestrator(state: ReviewState) -> ReviewState:
    ...
    # Sort
    severity_order = {DBSeverity.CRITICAL: 0, DBSeverity.ERROR: 1, DBSeverity.WARNING: 2, DBSeverity.INFO: 3}
    ...
    async with AsyncSessionLocal() as session:
        try:
            # Upsert Repository
            repo_query = await session.execute(select(Repository).where(Repository.owner == owner, Repository.repo_name == repo_name))
            ...
            pr_query = await session.execute(
                select(PullRequest).where(PullRequest.repository_id == repository.id, PullRequest.pr_number == pr_number)
            )
            ...
            review = Review(
                ...
                decision=decision,
                summary=summary,
                security_count=len([i for i in merged_list if i.category == "security"]),
            ...
            for issue in merged_list:
                ctype = CommentType.SECURITY if issue.category == "security" else CommentType.BUG if issue.category == "logic" else CommentType.STYLE
            ...
            logger.info(f"[Node: Orchestrator] Saved {len(merged_list)} comments to database.")
    ...
    logger.info(f"[Node: Orchestrator] Finished in {execution_time:.2f} seconds. Decision: {decision.name}, Score: {health_score}")
