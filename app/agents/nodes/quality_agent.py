...
async def quality_agent(state: ReviewState) -> ReviewState:
            ...
            if issue.rule in ("high_cyclomatic_complexity", "long_function"):
                ...
                for func in functions:
                    ...
                    if start_line == issue.line:
                        llm_issues = await llm_refactor.scan(file_path, func.get("name", ""), func.get("body", ""), start_line)
    ...
    summary = f"Found {total_issues} quality issues. Quality Score: {quality_score}/100."
