...
from app.core.metrics import prism_redis_hits_total
...
class RedisClient:
    ...
    async def is_duplicate_delivery(self, delivery_id: str) -> bool:
        ...
        if is_new:
            ...
            prism_redis_hits_total.labels(type="dedup", hit="false").inc()
            return False
        prism_redis_hits_total.labels(type="dedup", hit="true").inc()
        return True
    ...
    async def acquire_pr_lock(self, repo_name: str, pr_number: int) -> bool:
        ...
        locked = bool(await self.client.set(key, "locked", nx=True, ex=3600))
        prism_redis_hits_total.labels(
            type="lock", hit="true" if not locked else "false"
        ).inc()
        return locked
