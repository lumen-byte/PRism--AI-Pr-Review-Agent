...
async def process_pull_request(owner: str, repo: str, pr_number: int) -> None:
    ...
    if not has_lock:
        logger.info(
            f"Skipping PR {pr_number} in {owner}/{repo}: already being processed"
        )
        return
    ...
    try:
        ...
        logger.info(
            f"Successfully fetched PR {pr_metadata.pr_number} - {pr_metadata.title} by {pr_metadata.author}"
        )
        ...
        for file in changed_files:
            logger.info(
                f"File changed: {file.filename} ({file.status}) - +{file.additions} -{file.deletions}"
            )
        ...
        if diff:
            logger.info(
                f"Successfully fetched diff for PR {pr_number}, size: {len(diff)} bytes"
            )
        else:
        ...
        logger.info(
            f"Graph Execution Final State Node: {final_state.get('current_nodes')}"
        )

        await redis_client.set_pr_status(repo, pr_number, "COMPLETED")
        logger.info(
            f"Finished background processing for PR {pr_number} in {owner}/{repo}"
        )
