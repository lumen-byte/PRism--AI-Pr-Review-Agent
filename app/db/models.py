...
class PullRequest(Base):
    ...
    status: Mapped[PRStatus] = mapped_column(
        Enum(PRStatus, native_enum=False), nullable=False, index=True
    )
    created_at: Mapped[datetime] = mapped_column(
        DateTime, default=datetime.utcnow, index=True
    )
...
class Review(Base):
    ...
    pr_id: Mapped[uuid.UUID] = mapped_column(
        UUID(as_uuid=True),
        ForeignKey("pullrequest.id", ondelete="CASCADE"),
        nullable=False,
    )
    health_score: Mapped[float] = mapped_column(Float, nullable=False)
    decision: Mapped[ReviewDecision] = mapped_column(
        Enum(ReviewDecision, native_enum=False), nullable=False, index=True
    )
    ...
    publishing_status: Mapped[str] = mapped_column(
        String(50), server_default="PENDING", default="PENDING"
    )
    publishing_timestamp: Mapped[Optional[datetime]] = mapped_column(
...
class ReviewComment(Base):
    ...
    review_id: Mapped[uuid.UUID] = mapped_column(
        UUID(as_uuid=True), ForeignKey("review.id", ondelete="CASCADE"), nullable=False
    )
    file_path: Mapped[str] = mapped_column(String(512), nullable=False)
    line_number: Mapped[int] = mapped_column(Integer, nullable=False)
    comment_type: Mapped[CommentType] = mapped_column(
        Enum(CommentType, native_enum=False), nullable=False
    )
    severity: Mapped[Severity] = mapped_column(
        Enum(Severity, native_enum=False), nullable=False
    )
...
class User(Base):
    ...
    auth_provider: Mapped[str] = mapped_column(String(50), server_default="LOCAL", default="LOCAL")
